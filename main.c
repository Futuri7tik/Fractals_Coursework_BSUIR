...
// === FIX для совместимости raylib 5.0 + raygui ===
...
int main(void) {
    ...
    // === ВРЕМЕННЫЕ ПЕРЕМЕННЫЕ ДЛЯ GuiSlider (ОБЯЗАТЕЛЬНО!) ===
    ...
    while (!WindowShouldClose()) {
        ...
        if (state != STATE_GALLERY) {
            if (IsKeyPressedRepeat(KEY_UP)) camera.target.y -= camera_offset;
            if (IsKeyPressedRepeat(KEY_DOWN)) camera.target.y += camera_offset;
            if (IsKeyPressedRepeat(KEY_LEFT)) camera.target.x -= camera_offset;
            if (IsKeyPressedRepeat(KEY_RIGHT)) camera.target.x += camera_offset;
            ...
            // Ограничения камеры (защита от вылетов)
            if (camera.zoom < 0.1f) camera.zoom = 0.1f;
            if (camera.zoom > 100.0f) camera.zoom = 100.0f;
            if (camera_offset < 1.0f) camera_offset = 1.0f;
            if (camera_offset > 1000.0f) camera_offset = 1000.0f;
        }
        ...
        if (state == STATE_GALLERY) {
            // === ГАЛЕРЕЯ ===
            GuiPanel((Rectangle){WIDTH/2.0f - 300, HEIGHT/2.0f - 200, 600, 400}, "Fractal Gallery");
            camera.zoom = 1.0f;
            camera.target = (Vector2){WIDTH/2.0f, HEIGHT/2.0f};
            camera_offset = 20.0f;
            ...
            if (state == STATE_TREE) {
                ...
                // Используем ВРЕМЕННУЮ float переменную!
