...
void draw_line(sfRenderWindow* window,
    const double x_start, const double y_start,
    const double x_end, const double y_end,
    const sfColor color) {
    const sfVertex line[] = {
        {{(float) x_start, (float) y_start}, color},
        {{(float) x_end, (float) y_end}, color}
    };
...
void draw_tree(sfRenderWindow* window, const double x_start,
    ...
    draw_line(window, x_start, y_start, x_end, y_end, color);

    double new_length = length * parameters->length_factor;
    double angle_left = angle - parameters->angle;
    double angle_right = angle + parameters->angle;
